...
func SignIn(c *gin.Context) {
	...
	if err := database.DB.Raw("SELECT * FROM users WHERE email = ?", payload.Email).Scan(&user).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
    ...
    // expiration, err := strconv.ParseInt(config.Config("JWT_EXPIRATION_HOURS"), 10, 64)
